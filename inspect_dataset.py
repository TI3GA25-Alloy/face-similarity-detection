...
def get_semua_gambar(folder_path):
    return [f for f in os.listdir(folder_path) if f.endswith(VALID_EXT)]
...
def main():
    ...
    for idx, nama_folder in enumerate(folder_list[:20]):  # tampilkan 20 pertama
        ...
        n_selfies = (
            len(get_semua_gambar(path_selfies)) if os.path.exists(path_selfies) else 0
        )
    ...
    print(
        f"  Total semua foto       : {stats['total_foto_docs'] + stats['total_foto_selfies']}"
    )
    print()
    ...
    n_siap = (
        stats["total_orang"]
        - len(stats["orang_tanpa_docs"])
        - len(stats["orang_tanpa_selfies"])
    )

    print()
    print(f"   Orang siap dipakai (punya docs + selfies): ~{n_siap}")
    print(
        f"    Orang yang perlu dicek                  : {len(stats['orang_tanpa_docs']) + len(stats['orang_tanpa_selfies'])}"
    )
